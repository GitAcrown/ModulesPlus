...
class KarmaAPI:
    ...
    def update(self): # V0.02
        for server in self.data:
            if not "iam" in self.data[server]["META"]:
                self.data[server]["META"]["iam"] = {}
        return True
...
class Karma:
    ...
    @commands.command(pass_context=True)
    @checks.admin_or_permissions(manage_roles=True)
    async def roleselect(self, ctx, *roles):
        """Permet de définir des rôles que les membres pourront s'attribuer avec .iam"""
        if roles:
            if roles[0].lower() in ["reset", "stop", "off"]:
                self.karma.get_server(ctx.message.server, "META")["auto_roles"] = []
                self.karma.save(True)
                await self.bot.say("⚙ **Rôles attribuables par les membres** ─ Désactivée")
                return
            liste = []
            txt = ""
            for r in roles:
                if r in [r.name for r in ctx.message.server.roles]:
                    ...
                    role = discord.utils.get(ctx.message.server.roles, name=r)
                    txt += "• {}\n".format(role.mention)
            self.karma.get_server(ctx.message.server, "META")["iam"] = liste
            self.karma.save(True)
            em = discord.Embed(title="Rôles attribuables par les membres ─ Activé", description=txt)
            em.set_footer(text="Ne changez pas le nom des rôles fournis sous peine d'entraver son fonctionnement")
            await self.bot.say(embed=em)
        elif self.karma.get_server(ctx.message.server, "META")["iam"]:
            txt = ""
            for r in self.karma.get_server(ctx.message.server, "META")["iam"]:
                role = discord.utils.get(ctx.message.server.roles, name=r)
                txt += "• {}\n".format(role.mention)
            em = discord.Embed(title="Rôles attribuables par les membres ─ Liste", description=txt)
            em.set_footer(text="Tapez '.roleselect stop' pour désactiver cette fonctionnalité")
            await self.bot.say(embed=em)
        else:
            await self.bot.say("🚩 **Erreur** ─ Tapez la liste des __noms exacts__ des rôles entre guillemets pour activer cette fonction")
    ...
    async def iam(self, ctx, role: discord.Role = None):
        """Permet de recevoir ou retirer un rôle auto-attribuable"""
        roles = self.karma.get_server(ctx.message.server, "META")["iam"]
        if roles:
            if role:
                if role.name in roles:
                    if role not in ctx.message.author.roles:
                        await self.bot.add_roles(ctx.message.author, role)
                        await self.bot.say("{} **Rôle obtenu** ─ `{}`".format(ctx.message.author.mention, role.name))
                    else:
                        await self.bot.remove_roles(ctx.message.author, role)
                        await self.bot.say("{} **Rôle retiré** ─ `{}`".format(ctx.message.author.mention, role.name))
                else:
                    await self.bot.say("**Refusé** ─ Le rôle `{}` ne peut pas être attribué de telle"
                                       " façon.".format(role.name))
            else:
                txt = ""
                liste = []
                n = 1
                for r in roles:
                    role = discord.utils.get(ctx.message.server.roles, name=r)
                    txt += "**{}**. {}\n".format(n, role.mention)
                    liste.append([n, role])
                em = discord.Embed(title="Rôles attribuables par les membres ─ Liste", description=txt)
                em.set_footer(text="Tapez le numéro d'un rôle pour l'obtenir ou vous le retirer")
                msg = await self.bot.say(embed=em)
                rep = await self.bot.wait_for_message(author=ctx.message.author, channel=ctx.message.channel, timeout=30)
                if rep is None:
                    await self.bot.delete_message(msg)
                    return
                elif rep.content in [str(i[0]) for i in liste]:
                    await self.bot.delete_message(msg)
                    for i in liste:
                        if int(rep.content) == i[0]:
                            role = i[1]
                            if role in rep.author.roles:
                                await self.bot.remove_roles(rep.author, role)
                                await self.bot.say(
                                    "{} **Rôle retiré** ─ `{}`".format(rep.author.mention, role.name))
                            else:
                                await self.bot.add_roles(rep.author, role)
                                await self.bot.say(
                                    "{} **Rôle obtenu** ─ `{}`".format(rep.author.mention, role.name))
                            return
                    await self.bot.say("Ce numéro ne correspond à aucun rôle...")
                    return
                else:
                    await self.bot.say("Ce numéro ne correspond à aucun rôle...")
                    await self.bot.delete_message(msg)
            ...
            await self.bot.say("**Désactivé** ─ Aucun rôle n'est configuré pour être attribué de cette façon")
                            ...
                            if rep is None:
                                ...
                                if cible:
                                    ...
                                    if act is None or act.reaction.emoji == "↩":
                                        ...
                                        await self.bot.delete_message(msg)
                                        self.get_cache(server, "PRISON")[cible.id]["sortie"] = 0
